...
async def generate_knowledge_async_data(client: AsyncOpenAI,
    ...
    return [{"knowledge": choice.message.content,
             "answer": answer,
             "keyword": keyword,
             "question": question, "question_ids":question_id} for choice in choices]


async def generate_knowledge_async_data_all(records: list[str],
                                          client: AsyncOpenAI,
                                          batch_size:int=20,
                                          **kwargs) -> pd.DataFrame:
    all_knowledge = []
    for i in tqdm(range(0, len(records), batch_size), desc="Generating Answers"):
        answers = await asyncio.gather(
            *[generate_knowledge_async_data(client, **elem, **kwargs) for elem in records[i:i+batch_size]]
        )
...
def main(config: DictConfig):
    ...
    with asyncio.Runner() as runner:
        knowledge_df = runner.run(
            generate_knowledge_async_data_all(records, client, **config.generate_kwargs)
        )
